...
const map = {
    ...
    render(snakePointsArray, foodPoint, barrierPoint) {
        ...
        const barrier = this.cells[`x${barrierPoint.x}_y${barrierPoint.y}`];
        barrier.classList.add('barrier');
        this.usedCells.push(barrier);
    },
};
...
const game = {
    ...
    foodCounter() {
        ...
        counter.innerHTML = '';
        counter.insertAdjacentHTML('beforeend', this.getFoodCounterContent());
    },

    getFoodCounterContent() {
        const foodQuantity = this.snake.getBody().length;
        return `<h2>Счёт игры: ${foodQuantity}</h2>`
    },
    ...
    tickHandler() {
        if (!this.canMakeStep() || this.barrier.isOnPoint(this.snake.getNextStepHeadPoint())) {
            return this.finish();
        }
